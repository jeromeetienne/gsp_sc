...
class SuperType_(metaclass=_SuperTypeMeta):
...
if typing.TYPE_CHECKING:
    ...
    class SuperType(typing.Protocol):
        @classmethod
        def __class_getitem__(cls, params: typing.Any) -> type["SuperType"]: ...
...
if __name__ == "__main__":
    ...
    diffable_any: SuperType = DiffableNdarray(np.arange(4).reshape(2, 2))
    ndarray_any: SuperType = np.arange(3)
    transform_any: SuperType = TransformLinkBase()
    ...
    assert isinstance(diffable_any, SuperType)
    assert isinstance(ndarray_any, SuperType)
    assert isinstance(transform_any, SuperType)
    ...
    assert isinstance(shaped_diffable, SuperType[DType(float)])
    assert isinstance(shaped_diffable, SuperType[Shape(2, 3), float])
    assert isinstance(shaped_diffable_f32, SuperType[Shape(2, 3), DType(np.float32)])
    assert not isinstance(shaped_diffable_f32, SuperType[Shape(3, 2)])
    assert not isinstance(shaped_diffable_f32, SuperType[DType(np.float64)])
    ...
    def accepts_shaped_float(arr: SuperType[Shape(2, 3), float]) -> SuperType[Shape(2, 3)]:
        print("accepts_shaped_float:", arr)
        return arr

    def accepts_any(arr: SuperType) -> SuperType:
        print("accepts_any:", arr)
        return arr

    def accepts_transform(arr: SuperType[TransformLinkBase]) -> SuperType[TransformLinkBase]:
        print("accepts_transform:", arr)
        return arr

    def returns_shaped_float() -> SuperType[Shape(2, 3), float]:
    ...
    assert isinstance(returned, SuperType[Shape(2, 3), float])
