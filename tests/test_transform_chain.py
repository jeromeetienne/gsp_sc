import numpy as np
import pytest
from gsp.transform import TransformLinkImmediate, TransformLinkAssertShape, TransformLinkLambda
...
def test_transformchain_math_op_mul():
    arr = np.array([1, 2, 3])
    result = TransformLinkImmediate(arr).chain(TransformLinkLambda(lambda x: x * 5)).run()
